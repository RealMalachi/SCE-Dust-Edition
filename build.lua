...
-- Delete old ROM.
os.remove("ROM.prev.bin")

-- Backup the most recent ROM.
os.rename("ROM.bin", "ROM.prev.bin")

-- Assemble the ROM.
local assemble_result = common.assemble_file("Main.asm", "ROM.bin", tools.as, "", tools.s1p2bin, improved_dac_driver_compression and "" or " -a", false)
...
-- A successful build; we can quit now.
	os.exit(false)	--pause to see debug info